...
O Menu-OTRS **não consegue** escrever num separador normal só com HTTP. Com **WebDriver + JavaScript** (`HubBrowserDirectWrite`, padrão) injecta o mesmo script da consola **directamente no DOM**. Com **`HubWebDriverDebugAddress`** (ex.: `127.0.0.1:9222`) liga-se ao Chrome/Edge **já aberto** (`--remote-debugging-port=9222`) em vez de abrir janela nova.
...
3. WebDriver abre o URL do Gerador (ou usa separador já aberto), espera `.ticket-form`, executa **JavaScript** no DOM (`HubBrowserDirectWrite`) ou SendKeys se desactivado.
...
No `config.json` defina **`HubWebDriverEnabled`**: `true`, **`HubBrowserDirectWrite`**: `true` (padrão), **`HubWebDriverBrowser`**: `Chrome` ou `Edge`. Opcional: **`HubWebDriverDebugAddress`**: `127.0.0.1:9222` para ligar ao browser existente.

Com **`HubWebDriverAutoFill`**: `true`, o preenchimento directo corre sem pergunta `s/N`.

Selenium: Gallery (`Install-Module Selenium -Scope CurrentUser`), **`tools\Selenium\`** ou **`HubSeleniumModulePath`** — ver **`tools/Selenium/README.md`**.
